...
results = os.path.join(results, 'RMSE-Milnes-n1')
...
for drug in plot_drugs:
    ...
    for i, m in enumerate(models[:13]):
        ...
        for a, b in enumerate(baseline_models):
            if b == 'lei':
                RMSE = np.loadtxt(path2fits + f'/{drug}-m{m}-errors.txt')
            elif b == 'fda':
                RMSE = np.loadtxt(path2fits + f'/{drug}-li-m{m}-errors.txt')
            else:
    ...
    fig, (a0, a1) = plt.subplots(1, 2, gridspec_kw={'width_ratios': [1, 4]}, figsize=(10, 6), sharey=True)
    plt.title(drug)
    ...
    a1.scatter(models[1:9], RMSEs[0][1:9], marker='o', s=100, color='none', linewidth=1.5, \
        edgecolor=colors[0], label=r'2 params: $k_{\rm{on}}$, $k_{\rm{off}}$')
    a1.scatter(models[9], RMSEs[0][9], marker='o', s=100, color='none', linewidth=1.5, \
        edgecolor=colors[1], label=r'3 params: $k_{\rm{on}}$, $k_{\rm{off}}$, $k_{\rm{trap}}$')
    a1.scatter(models[10], RMSEs[0][10], marker='o', s=100, color='none', linewidth=1.5, \
        edgecolor=colors[2], label=r'4 params: $k_{\rm{on, O}}$, $k_{\rm{off, O}}$, $k_{\rm{on, I}}$, $k_{\rm{off, I}}$')
    a1.scatter(models[11:13], RMSEs[0][11:13], marker='o', s=100, color='none', linewidth=1.5, \
        edgecolor=colors[3], label=r'3 params: $k_{\rm{on, O}}$, $k_{\rm{off, O}}$, $k_{\rm{on, I}}$')
    a1.scatter(models[13], RMSEs[0][13], marker='o', s=100, color='none', linewidth=1.5, \
        edgecolor=colors[4], label=r'4 params: $k_{\rm{on, O}}$, $k_{\rm{off, O}}$, $k_{\rm{on, I}}$, $k_{\rm{trap}}$')
    a1.scatter(models[14], RMSEs[0][14], marker='o', s=100, color='none', linewidth=1.5, \
        edgecolor=colors[5], label=r'4 params: $k_{\rm{on}}$, $k_{\rm{off}}$, $k_{\rm{untrap}}$, $k_{\rm{trap}}$')
    a1.scatter(models[15], RMSEs[0][15], marker='o', s=100, color='none', linewidth=1.5, \
        edgecolor=colors[6], label=r'4 params: $\hat{k}_{\rm{on}}$, $k_{\rm{off}}$, $\rm{EC}_{50}$, $V_{\rm{1/2, trap}}$')
    a1.scatter(models[16], RMSEs[0][16], marker='o', s=100, color='none', linewidth=1.5, \
        edgecolor=colors[7], label=r'3 params: $k_{\rm{on}}$, $k_{\rm{off}}$, $V_{\rm{1/2, trap}}$')
    ...
    a1.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0., fontsize=8)
